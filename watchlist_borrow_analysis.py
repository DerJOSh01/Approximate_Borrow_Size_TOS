...
HEADER_CANDIDATE_PATTERNS = [
    ...
    r"^price$", r"^last$", r"^close$", r"^kurs$", r"^preis$", r"^mark$",
...
def plot_grid_overview(all_df: pd.DataFrame, topN: int = 9, x_mode: str = "time", overview_rank: str = "dev_cum"):
    # ---- Ranking vorbereiten ----
    symbols = []
    if overview_rank == "borrow":
        # klassisch: Summe Borrow Size
        rank = (all_df.groupby("Symbol")["Borrow"].sum().sort_values(ascending=False))
        symbols = list(rank.head(topN).index)
        rank_desc = "Top {n} by Borrow Size"
    else:
        # dev_cum: aktuelle Abweichung |Price_last - VWAbP_cum_last|
        dev_rows = []
        for sym, g in all_df.groupby("Symbol"):
            ...
            vw_cum = (cum_num / cum_den).astype(float)
            # letztes valides Paar
            valid = (~vw_cum.isna()) & (~g["Price"].isna())
            if not valid.any():
                dev = np.nan
                ts_last = g["Stamp"].max()
            else:
                idx_last = valid[valid].index[-1]
                dev = abs(float(g.loc[idx_last, "Price"]) - float(vw_cum.loc[idx_last]))
                ts_last = g.loc[idx_last, "Stamp"]
            dev_rows.append((sym, dev, ts_last))
        dev_df = pd.DataFrame(dev_rows, columns=["Symbol","DevCum","StampLast"])
        # Symbole mit NaN-Dev nach hinten
        dev_df["DevSort"] = dev_df["DevCum"].fillna(-1)  # -1 → wird am Ende durch sort_values(ascending=False) landen
        dev_df = dev_df.sort_values(["DevSort","StampLast"], ascending=[False, False])
        symbols = dev_df["Symbol"].head(topN).tolist()
        rank_desc = "Top {n} by |Price - VWAbP(cum.)| (latest)"
    ...
    fig.suptitle(f"Price & Borrow Size Overview — {rank_desc.format(n=n)} — last {all_df['Date'].nunique()} days", fontsize=14)
    ...
    for ax, sym in zip(axes.flatten(), symbols):
        ...
        vw_cum = (cum_num / cum_den).astype(float)
        if x_mode == "time":
            ax.plot(s["Stamp"], vw_cum, linestyle=":", linewidth=1.2, label="VWAbP (cum.)")
            # dezentes Füllen für Lesbarkeit
            ax.fill_between(s["Stamp"], s["Price"], vw_cum, alpha=0.07)
        else:
            ax.plot(xs, vw_cum.values, linestyle=":", linewidth=1.2, label="VWAbP (cum.)")
            ax.fill_between(xs, s["Price"].values, vw_cum.values, alpha=0.07)
...
def plot_detail_navigator(
    ...
    borrow_floor_mode: str = "none",   # none | p10 | min
):
    symbols = sorted(all_df["Symbol"].dropna().unique())
    if not symbols:
        print("No symbols available for detail view.")
        return
    ...
    def draw(symbol):
        ...
        if x_mode == "time":
            width = _adaptive_bar_width(s["Stamp"])
            ax_price.plot(s["Stamp"], s["Price"], linewidth=1.4, marker="o", markersize=3.2, label=f"{symbol} Price")
            _format_time_axis(ax_price)
        else:
            xs = list(range(len(s)))
            ax_price.plot(xs, s["Price"].values, linewidth=1.4, marker="o", markersize=3.2, label=f"{symbol} Price")
            _format_index_axis(ax_price, _labels_from_stamps(s["Stamp"].tolist()))
        ax_price.set_ylabel("Price")

        # VWAbP overall + cumulative
        v = (s["Price"] * s["Borrow"]).sum() / s["Borrow"].sum() if s["Borrow"].sum() else float("nan")
        # if pd.notna(v):
        #     ax_price.axhline(v, linestyle="--", linewidth=1, label="VWAbP (overall)")
        # --- Rolling VWAbP (robust, ohne pd.NA) ---
        N = 10  # Fenstergröße nach Wunsch
        ...
        # 0 im Nenner -> np.nan (nicht pd.NA!)
        den = den.where(den != 0.0, np.nan)

        roll = (num / den).astype(float)                 # -> float-Serie mit np.nan
        roll = roll.replace([np.inf, -np.inf], np.nan)   # Sicherheit

        # Nur plotten, wenn nicht alles NaN ist
        if roll.notna().any():
            if x_mode == "time":
                ax_price.plot(s["Stamp"], roll.values, linestyle="--", linewidth=1.2,
                            label=f"VWAbP ({N}-roll)")
            else:
                ax_price.plot(range(len(roll)), roll.values, linestyle="--", linewidth=1.2,
                            label=f"VWAbP ({N}-roll)")

        cum_num = (s["Price"] * s["Borrow"]).cumsum()
        cum_den = s["Borrow"].cumsum().replace(0, pd.NA)
        vw_series = (cum_num / cum_den)
        if x_mode == "time":
            ax_price.plot(s["Stamp"], vw_series, linestyle=":", linewidth=1.2, label="VWAbP (cum.)")
        else:
            xs = np.arange(len(vw_series))
            ax_price.plot(xs, vw_series.values, linestyle=":", linewidth=1.2, label="VWAbP (cum.)")
        ...
        info_txt = (
            ...
            f"VWAbP: {v:,.2f}" if pd.notna(v) else f"[{symbol}]  (VWAbP: N/A)"
        )
        ax_info.text(0.01, 0.5, info_txt, fontsize=10, va="center")

        fig.suptitle("Detail View — A/D or ←/→  |  Z: Δ  |  +/−: zoom  |  X: floor (none/p10/min)  |  Q: quit", fontsize=12)
        fig.tight_layout()
...
def main():
    ap = argparse.ArgumentParser(description="Analyze Price vs Borrow Size over last N days (autoscaled).")
    ...
    ap.add_argument("--overview-rank", choices=["borrow", "dev_cum"], default="dev_cum",
                help="Ranking im Overview: 'borrow' (Sum Borrow Size) oder 'dev_cum' (aktuelle Abweichung |Price - VWAbP_cum|).")
    ...
    plot_grid_overview(all_df, topN=args.topN, x_mode=args.x_mode, overview_rank=args.overview_rank)
